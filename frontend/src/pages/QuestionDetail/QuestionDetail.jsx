...
import AIModalData from "../../components/AIModalData/AIModalData";
...
const QuestionDetail = () => {
  ...
  const [showAIModalData, setShowAIModalData] = useState(null);
  ...
  const fetchAIAnalysis = async () => {
    try {
      const data = await questionService.getAIAnalysisByQuestion(questionId);
      setAIAnalysis(data);
    } catch (err) {
      console.error("Error fetching AI analysis:", err);
      toast.error("Failed to fetch AI analysis. Please try again later.");
    } finally {
      setLoading(false);
    }
  };

  const handleShowAI = (questionId) => {
    console.log("AI Analysis Data:", aiAnalysis, questionId);
    if (aiAnalysis) {
        
      const aiData = aiAnalysis["moderation"]["answers"].find(
        (item) => item.answer_id === questionId
      );
      setShowAIModal(true);
      console.log("AI Data for Question:", aiData);
      setShowAIModalData(aiData);
    }
  };
  ...
  const handleVote = async (answerId, voteType) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://127.0.0.1:5000/answers/${answerId}/vote`,
        {
          method: "POST",
          headers: {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      const user_id = user?._id || user?.id || user?.user_id;
      const res = await fetch(
        `http://127.0.0.1:5000/questions/${questionId}/answers`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="vote-buttons">
      <button
        className={`vote-button vote-button--up${
          userUpvoted ? " vote-button--active vote-button--green" : ""
        }`}
      ...
      <button
        className={`vote-button vote-button--down${
          userDownvoted ? " vote-button--active vote-button--red" : ""
        }`}
    ...
    <div className="answer-card">
      <div className="answer-card__content">
        ...
        <div className="answer-card__main">
          ...
          />
          <div className="answer-card__meta-row">
            ...
            {isAdmin && (
              <button
                ...
                onClick={() => handleShowAI(answer._id)}
              >
                AI Analysis
              </button>
            )}
  ...
  function renderYourAnswerSection() {
    {
      /* AI Analysis Section */
    }
    if (!isAdmin) {
  ...
  return (
    <div className="question-detail">
      <div className="question-detail__container">
            ...
            <div className="question-detail__info">
              ...
              <div className="question-detail__tags">
                {question.tags.map((tag) => (
                  <span key={tag} className="question-detail__tag">
                    {tag}
                  </span>
                ))}
              </div>
        ...
        {/* AIModal for admin AI analysis */}

        {isAdmin && (
          <AIModalData
            isOpen={showAIModal}
            onClose={() => setShowAIModal(false)}
            data={showAIModalData}
          />
        )}

        {/* Your Answer Section */}
        {renderYourAnswerSection()}
      </div>
    </div>
  );
};
